...
public class FirstBattleScreen extends Screen {
    ...
    public FirstBattleScreen() {
        this("FIRST_BATTLE", "POST_FIRST_BATTLE");
    }
    ...
    public void prepare() {
        ...
        BufferedImage enemy1Sprite = Resources.images().get("Sprites/Inimigos/Carniçal.png");
        BufferedImage enemy2Sprite = Resources.images().get("Sprites/Inimigos/Mao.png");

        enemies.clear();
        enemies.add(new Enemy("Carniçal",         enemy1Sprite, 50));
        enemies.add(new Enemy("Garra Rastejante",  enemy2Sprite, 60));
    ...
    private void changeTarget(int dir) {
        if (!Game.screens().current().getName().equals(getName())) return;
        if (state != BattleState.PLAYER_CHOOSING_TARGET) return;
    ...
    private void confirmAction() {
        if (state == BattleState.PLAYER_TURN && menuSelection == 0) {
            state = BattleState.PLAYER_CHOOSING_TARGET;
            selectedEnemyIndex = 0;
            while (!enemies.get(selectedEnemyIndex).alive && selectedEnemyIndex < enemies.size() - 1)
                selectedEnemyIndex++;
    ...
    private void performEnemyTurns() {
        for (int i = 0; i < enemies.size(); i++) {
            ...
            Game.loop().perform(200 + i * 200, () -> {
                // Se o jogo já acabou, não processa turno
    ...
    private boolean allEnemiesDead() { return enemies.stream().noneMatch(e -> e.alive); }
    ...
    public void render(Graphics2D g) {
        ...
        for (int i = 0; i < enemies.size(); i++) {
            ...
            if (e.sprite != null) g2.drawImage(e.sprite, enemyX, ey, null);
                ...
                g2.fillRoundRect(enemyX, ey, ew, eh, 10, 10);
            }
    ...
    private void renderBattleMenu(Graphics2D g, int sw, int sh) {
        ...
        if (state == BattleState.PLAYER_CHOOSING_TARGET) {
            g.setFont(new Font("SansSerif", Font.PLAIN, 14)); g.setColor(GOLD);
            g.drawString("Escolha o inimigo  ← →  e confirme (Enter)", tx, ty + 30);
        }
    ...
    @Override public void suspend() { super.suspend(); }
}