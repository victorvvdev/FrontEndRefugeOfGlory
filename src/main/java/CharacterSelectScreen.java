...
import util.CharacterServiceClient;
...
public class CharacterSelectScreen extends Screen implements MouseListener, MouseMotionListener {
    ...
    // Feedback de erro exibido abaixo do botão
    private volatile String  feedback   = "";
    private volatile boolean criando    = false; // evita duplo clique / dupla requisição
    ...
    @Override public void mouseExited(MouseEvent e)   { hoverBarbaro = hoverEspadachim = hoverConfirmar = false; }
    ...
    // -------------------------------------------------------------------------
    // Lógica de confirmação — chama a API em thread separada
    // -------------------------------------------------------------------------
    ...
    private void confirmarSelecao() {
        ...
        String name           = selectedCharacter.equals("BARBARO") ? "Bárbaro" : "Espadachim";
        String characterClass = data.getCharacterClassForApi();

        new Thread(() -> {
            try {
                long characterId = CharacterServiceClient.createCharacter(name, characterClass);
                if (characterId > 0) {
                    data.setCharacterId(characterId);
                    data.setCharacterName(name);
                    System.out.println("Personagem criado com ID: " + characterId);
                    Game.screens().display("CHARACTER_STORY");
                } else {
                    feedback = "Erro ao criar personagem. Tente novamente.";
                    criando = false;
                }
            } catch (Exception ex) {
                feedback = "Erro de conexão: " + ex.getMessage();
                System.out.println("[CharacterSelectScreen] " + ex.getMessage());
                criando = false;
            }
        }).start();
    }
    ...
    // -------------------------------------------------------------------------
    // Render — idêntico ao original, só adiciona o feedback de erro
    // -------------------------------------------------------------------------
    ...
    public void render(Graphics2D g) {
        ...
        // Feedback de erro abaixo do botão
        if (!feedback.isBlank()) {
            g2.setFont(new Font("SansSerif", Font.ITALIC, 12));
            g2.setColor(COLOR_ERROR);
            FontMetrics fm = g2.getFontMetrics();
            double fbX = (sw - fm.stringWidth(feedback)) / 2.0;
            double fbY = btnConfirmarBounds.getY() + btnConfirmarBounds.getHeight() + 22;
            g2.drawString(feedback, (int)fbX, (int)fbY);
        }
    ...
    private void renderConfirmButton(Graphics2D g, boolean hovered) {
        ...
        g.drawString(txt,
                (int)(x + (w - fm.stringWidth(txt)) / 2.0),
                (int)(y + (h + fm.getAscent() - fm.getDescent()) / 2.0 - 1));
    }
}