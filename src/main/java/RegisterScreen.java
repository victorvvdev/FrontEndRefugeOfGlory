...
public class RegisterScreen extends Screen {
    ...
    private static final Color COLOR_OK     = new Color(100, 200, 100);
    ...
    // Feedback exibido no card (vazio = nenhum)
    private volatile String feedback   = "";
    private volatile boolean feedbackOk = false;
    ...
    private GuiComponent buildBtnFinalizarCadastro(double x, double y, double w, double h) {
        GuiComponent btn = new GuiComponent(x, y, w, h) {
            ...
            public void render(Graphics2D g) {
                ...
                g2.drawString(txt,
                        (int)(getX() + (getWidth() - fm.stringWidth(txt)) / 2.0),
                        (int)(getY() + (getHeight() + fm.getAscent() - fm.getDescent()) / 2.0 - 1));
                g2.dispose();
            }
        };

        btn.onClicked(ignored -> {
            ...
            // Validações locais
            if (username.isBlank() || email.isBlank() || senha.isBlank()) {
                setFeedback("Preencha todos os campos.", false);
                return;
            }
            if (!email.contains("@")) {
                setFeedback("E-mail inválido.", false);
                return;
            }
            if (!senha.equals(confirma)) {
                setFeedback("As senhas não coincidem.", false);
                return;
            }

            // Chamada à API em thread separada para não travar o render
            new Thread(() -> {
                try {
                    boolean ok = AuthServiceClient.register(username, senha, email);
                    if (ok) {
                        setFeedback("Conta criada! Redirecionando...", true);
                        Thread.sleep(1200);
                        Game.screens().display("LOGIN");
                    } else {
                        setFeedback("Usuário ou e-mail já cadastrado.", false);
                    }
                } catch (Exception e) {
                    setFeedback("Erro de conexão: " + e.getMessage(), false);
                }
            }).start();
        });
    ...
    private void renderCard(Graphics2D g) {
        ...
        if (!feedback.isBlank()) {
            ...
            g.setColor(feedbackOk ? COLOR_OK : COLOR_ERROR);
            FontMetrics fm = g.getFontMetrics();
            g.drawString(feedback,
                    (int)(cardX + (CARD_W - fm.stringWidth(feedback)) / 2.0),
                    (int)(cardY + CARD_H - 12));
        }
    }
