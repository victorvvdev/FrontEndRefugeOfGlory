        ...
        btn.onClicked(ignored -> {
            ...
            try {
                ...
                if (token != null) {
                    ...
                    System.out.println("Login bem‑sucedido! Token obtido.");
                    Game.screens().display("INTRO_NARRATIVE");
                } else {
                    System.out.println("Login falhou: credenciais inválidas.");
                }
            } catch (Exception e) {
                System.out.println("Erro ao conectar ao auth‑service: " + e.getMessage());
            }
        });
        return btn;
    }
